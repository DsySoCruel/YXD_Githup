...
//  YXDApp
//
//  Created by daishaoyang on 2017/12/21.
...
@interface SearchDisplayController ()<UISearchResultsUpdating,UISearchBarDelegate,UISearchControllerDelegate,UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate,SearchHistoryCellDelegate>
...
- (void)viewDidLoad {
    ...
    self.searchController.searchBar.placeholder = @"请输入商家、分类或商圈";
...
//    self.resultsTableController.tableView.delegate = self;
    ...
    if (self.keyword.length) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [self searchGoodMessagesByKeyWord:self.keyword];
        });
    }
}
//取消searchbar背景色
...
//----------------过滤 --进行跳转搜索
- (void)searchGoodMessagesByKeyWord:(NSString *)keyWord{
//    //正则表达式, 只过滤中文
//    NSString *urlString = keyWord;
//    NSError *error;
//    //^[\u4e00-\u9fa5_a-zA-Z0-9]+$ 这是正则表达式, 仅支持中文, 字母, 下划线, 数字
//    NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:@"^[\u4e00-\u9fa5_a-zA-Z0-9]+$" options:0 error:&error];
//    if (regex != nil) {
...
//            if (result.length) {
//                NSString *strTitle = [result stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
...
//                searchVC.keyWord = strTitle;
//                [self presentViewController:navi animated:NO completion:nil];
////                [self.navigationController pushViewController:searchVC animated:YES];
//            }
//        } else {
//            [MBHUDHelper showError:@"仅支持中文,字母,下划线,数字"];
//        }
//    }
