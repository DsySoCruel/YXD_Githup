...
- (NSString *)getNewTimeFromDurationSecond:(NSInteger)duration {
    NSString *newTime;
    if (duration < 10) {
        newTime = [NSString stringWithFormat:@"0:0%tu",duration];
    } else if (duration < 60) {
        newTime = [NSString stringWithFormat:@"0:%tu",duration];
    } else {
        NSInteger min = duration / 60;
        NSInteger sec = duration - (min * 60);
        if (sec < 10) {
            newTime = [NSString stringWithFormat:@"%tu:0%tu",min,sec];
        } else {
            newTime = [NSString stringWithFormat:@"%tu:%tu",min,sec];
        }
    }
    return newTime;
}
