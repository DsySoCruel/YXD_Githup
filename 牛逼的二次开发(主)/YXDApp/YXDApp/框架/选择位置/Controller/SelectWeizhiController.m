...
@interface SelectWeizhiController ()<AMapSearchDelegate,UITableViewDelegate,UITableViewDataSource,UISearchBarDelegate>
...
@property (nonatomic,strong) WeiZhiModel *tempWeiZhiModel;//用于传值
@property (nonatomic,strong) WeiZhiModel *currentWeizhiModel;//用于定位到当前

@end

@implementation SelectWeizhiController
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.tempWeiZhiModel = [WeiZhiModel new];
    self.title = @"选择位置";
    self.navigationItem.rightBarButtonItem = [UIBarButtonItem itemtitle:@"定位到当前" target:self action:@selector(dingweiAction)];
    ...
    [self getWeizhi];
    //1.先定位城市 判断是否开通
    //1.开通后 进行搜索历史纪录  2.没有开通显示请选择
}

//获取历史搜索记录
//1.定位获取到的开通城市 /获取历史搜索记录  /获取搜索定位信息位置
//2.选择城市 /获取历史搜索记录 /获取搜索定位信息位置
- (void)getHistoryData{
//    WeiZhiModel *bmodel = [WeiZhiManager sharedInstance].weizhiModel;
    ...
    parames[@"areaId2"] = self.tempWeiZhiModel.areaId2;
    WeakObj(self);
...
- (void)setupUI{
    ...
    [self.selectCityButton setTitle:@"定位中" forState:UIControlStateNormal];
...
- (void)getWeizhi{
    ...
    [getCoordinate getCoordinateToolActionWith:^(GetCoordinateModel *model) {
        ...
        //保存当前定位城市
        self.currentWeizhiModel = [WeiZhiModel new];
        self.currentWeizhiModel.latitude = parames[@"lat"];
        self.currentWeizhiModel.longitude = parames[@"lng"];
        self.currentWeizhiModel.city = model.city;
        self.currentWeizhiModel.xiaoqu = model.xiaoqu;
        ...
        [[NetWorkManager shareManager] POST:USER_checkCity parameters:parames successed:^(id json) {
            if (json) {
                //当前城市已经开通
                Weakself.tempWeiZhiModel = [WeiZhiModel mj_objectWithKeyValues:json];
                Weakself.tempWeiZhiModel.xiaoqu = model.xiaoqu;
                [Weakself.selectCityButton setTitle:Weakself.tempWeiZhiModel.city forState:UIControlStateNormal];
//                [WeiZhiManager sharedInstance].weizhiModel = getModel;
                Weakself.currentWeizhiModel.areaId2 =  Weakself.tempWeiZhiModel.areaId2;

                [Weakself getHistoryData];//获取历史搜索记录
            }else{
                //当前城市没有开通
                [Weakself.selectCityButton setTitle:@"请选择" forState:UIControlStateNormal];
            }
        } failure:^(NSError *error) {
            [Weakself.selectCityButton setTitle:@"请选择" forState:UIControlStateNormal];
...
- (void)weizhiButtonAction:(UIButton *)sender{
    ...
    selectCityController.selectCityBlock = ^(NSString *cityName, NSString *cityId) {
        ...
        Weakself.tempWeiZhiModel.city = cityName;
        Weakself.tempWeiZhiModel.areaId2 = cityId;
        //设置对应的位置信息
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText{
    ...
    if (![self.selectCityButton.titleLabel.text isEqualToString:@"请选择"]) {
        request.city                = self.tempWeiZhiModel.city;
    }
    //    request.types               = @"高等院校";
...
- (void)dingweiAction{
    WeiZhiModel *bmodel = [WeiZhiManager sharedInstance].weizhiModel;
    bmodel.latitude = self.currentWeizhiModel.latitude;
    bmodel.longitude = self.currentWeizhiModel.longitude;
    bmodel.xiaoqu = self.currentWeizhiModel.xiaoqu;
    bmodel.areaId2 = self.currentWeizhiModel.areaId2;
    [WeiZhiManager sharedInstance].weizhiModel = bmodel;
    if (self.selectCityBlock) {
        self.selectCityBlock();
    }
