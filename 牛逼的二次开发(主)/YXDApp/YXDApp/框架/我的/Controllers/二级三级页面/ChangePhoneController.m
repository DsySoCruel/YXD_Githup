...
- (void)setUpUI{
    ...
    LoginTextField *phoneNumTF = [LoginTextField createLineTextFieldWithHolderStr:@"请输入新手机号码" image:IMAGECACHE(@"icon_5")
...
- (void)LoginAction:(UIButton *)sender{
    ...
    WeakObj(self);
    [[NetWorkManager shareManager] POST:USER_updataPhone parameters:parames successed:^(id json) {
        if (json) {
            [MBHUDHelper showSuccess:@"手机号修改成功"];
            
            YXDUserInfoModel *account = [YXDUserInfoStore sharedInstance].userModel;
            account.loginName = Weakself.phoneNumTF.text;
