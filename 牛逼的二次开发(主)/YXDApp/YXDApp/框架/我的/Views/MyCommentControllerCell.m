...
@interface MyCommentControllerCell()

@property (nonatomic,strong) UIView *firstView;
@property (nonatomic,strong) UIImageView *iconShopView;
...
@implementation MyCommentControllerCell
...
- (void)setupUI{
    ...
    self.iconShopView = [[UIImageView alloc] initWithImage:IMAGECACHE(@"icon_46")];
    [self.iconShopView setContentHuggingPriority
     :UILayoutPriorityDefaultHigh forAxis:UILayoutConstraintAxisHorizontal];
    //添加手势
    UITapGestureRecognizer *tap1 = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(pushToShopController)];
    [self.firstView addGestureRecognizer:tap1];


    [self.firstView addSubview:self.iconShopView];
    self.shopNameLabel = [UILabel new];
    self.shopNameLabel.text = @"沃尔玛-花园路店";
    self.shopNameLabel.font = LPFFONT(12);
    [self.firstView addSubview:self.shopNameLabel];
    self.jiantouImageView = [[UIImageView alloc] initWithImage:IMAGECACHE(@"icon_55")];
    [self.firstView addSubview:self.jiantouImageView];
...
- (void)setupLayout{
    ...
    [self.iconShopView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.offset = 0;
        make.left.offset = 10;
    }];
    [self.shopNameLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.offset = 0;
        make.left.equalTo(self.iconShopView.mas_right).offset = 5;
    ...
    [self.secondView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.firstView.mas_bottom).offset = 1;
        make.left.right.offset = 0;
...
- (void)setModel:(MyCommentModel *)model{
    ...
    self.userNameLabel.text = model.userLogin;
